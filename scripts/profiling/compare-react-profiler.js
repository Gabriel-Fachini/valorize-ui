#!/usr/bin/env node
/**
 * Script de comparação de performance React DevTools Profiler
 * 
 * Uso: node scripts/compare-react-profiler.js <antes.json> <depois.json>
 */
  ...
  roots.forEach(root => {
    ...
    commitData.forEach(commit => {
      ...
      fiberActualDurations.forEach(([fiberId, duration]) => {
        ...
        // Identifica contextos baseado no fiberId (assumindo que contextos têm IDs específicos)
        // Vamos usar uma abordagem mais simples focando no total
...
function compareMetrics(before, after) {
  ...
  console.log('\n' + '-'.repeat(100))
  console.log('🎯 ANÁLISE DOS CONTEXTOS\n')

  const contextNames = ['theme', 'sidebar', 'accessibility']
  contextNames.forEach(contextName => {
    const beforeCtx = before.contexts[contextName]
    const afterCtx = after.contexts[contextName]
    ...
    if (beforeCtx && afterCtx) {
      ...
      if (parseFloat(ctxDiff) < -10) {
        ...
        console.log(`  ⚠️ Sem melhoria ou piora`)
      }
    }
  })
...
if (process.argv.length < 4) {
  console.error('Uso: node compare-react-profiler.js <antes.json> <depois.json>')
  process.exit(1)
}
